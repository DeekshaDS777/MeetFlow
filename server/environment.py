...
class MeetFlowEnvironment:
    ...
    def _observation(self) -> Observation:
        ...
        if self.repeat_count >= 2:
            notes.append("Avoid repeating the same action or same item in the same stage.")
    ...
    def reset(self, session_id: Optional[str] = None) -> Observation:
        ...
        self.last_stage_item: Optional[str] = None
        self.repeat_count = 0
    ...
    def _stage_item_key(self, action: Action) -> str:
        return f"{action.action_type}::{self._norm(action.item)}"
    ...
    def step(self, action: Action) -> tuple[Observation, Reward]:
        if self.done:
            ...
            return obs, Reward(
                ...
                score=self._bounded_score(self.score),
        ...
        stage_item_key = self._stage_item_key(action)

        exact_repeat = signature == self.last_signature
        stage_item_repeat = stage_item_key == self.last_stage_item

        repeated = exact_repeat or stage_item_repeat
        self.repeat_count = self.repeat_count + 1 if repeated else 0

        self.last_signature = signature
        self.last_stage_item = stage_item_key
        ...
        self.score = self._bounded_score(scoring["score"])
        ...
        reward_value = self._bounded_score(reward_value)
